...
import { arbiColor, arbiMainnet, arbiScanTx, bnToNumber, bnToNumberBTC, deposit, endpoint, ethColor, etherScanTx, ethMainnet, exercise, http,purchase, roundTo5, withdraw } from "../utils/utils";
import { getDAIPrice, getWbtcPrice } from "./chainlink-price";
import { BigNumber } from "ethers";
const NodeCache = require("node-cache");
var cron = require('node-cron');

export default class wbtcdai {
    ...
    try {
      if (actions === deposit) {
        ...
        if (unit == this.pair.split("/")[0]) {
          const priceNow = roundTo5(await getWbtcPrice());
      ...
      if (actions === exercise) {
        ...
        let content;
        const priceSizeNow = roundTo5((await getWbtcPrice()) * <number>eventData.contractSize);
        if (priceSizeNow >= parseInt(envConfig.filterSizePrice)) {
