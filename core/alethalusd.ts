...
import { getDAIPrice, getEthPrice } from "./chainlink-price";
import { BigNumber } from "ethers";
const NodeCache = require("node-cache");
var cron = require('node-cron');

export default class alethalusd {
    ...
    try {
      if (actions === deposit) {
        ...
        if (unit == this.pair.split("/")[0]) {
          const priceNow = roundTo5(await getEthPrice());
      ...
      if (actions === exercise) {
        ...
        let content;
        const priceSizeNow = roundTo5((await getEthPrice()) * <number>eventData.contractSize);
        if (priceSizeNow >= parseInt(envConfig.filterSizePrice)) {
