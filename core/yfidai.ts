...
import { getDAIPrice,getYfiPrice } from "./chainlink-price";
import { BigNumber } from "ethers";

export default class yfidai {
  pair = 'YFI/DAI';
    ...
    try {
      if (actions === deposit) {
        ...
        if (unit == this.pair.split("/")[0]) {
          const priceNow = roundTo5(await getYfiPrice());
      ...
      if (actions === exercise) {
        ...
        let content;
        const priceSizeNow = roundTo5((await getYfiPrice()) * <number>eventData.contractSize);
        if (priceSizeNow >= parseInt(envConfig.filterSizePrice)) {
